...
import { NoteLimitError, SyncNotesRepository } from "../../src/lib/storage/SyncNotesRepository";
...
describe("SyncNotesRepository", () => {
  ...
  it("creates a note and records it in the index", async () => {
    ...
    expect(await repo.list()).toEqual([{ id: "id-1", title: "First", updatedAt: note.updatedAt }]);
  });
  ...
  it("saves body changes and refreshes updatedAt + index", async () => {
    ...
    const returned = await repo.save({ ...note, body: "hello", title: "Renamed" });
    expect(returned).toMatchObject({ id: note.id, body: "hello", updatedAt: note.updatedAt + 5000 });
  ...
  it("deletes a note and its index entry", async () => {
    ...
    expect(await repo.list()).toEqual([{ id: b.id, title: "B", updatedAt: b.updatedAt }]);
  });
